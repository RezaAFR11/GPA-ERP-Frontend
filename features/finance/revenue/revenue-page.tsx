...
export default function RevenuePage() {
  ...
  const { data: revenueData, isLoading } = useQuery({
    queryKey: ["receivables", "cash-collection", paymentFilter, debouncedSearch, sortKey, sortDirection, page],
    queryFn: ({ signal }) => receivablesApi.list({
      ...receivableFilters,
      sort_by: sortKey,
  ...
  const { data: summary } = useQuery({
    queryKey: ["receivables", "summary", paymentFilter, debouncedSearch],
    queryFn: ({ signal }) => receivablesApi.summary(receivableFilters, signal).then((r) => r.data),
  });

  const { data: projects = [] } = useQuery({
    queryKey: ["projects", "lookup", "revenue"],
    queryFn: ({ signal }) => projectsApi.lookup(signal).then((r) => r.data),
  });
